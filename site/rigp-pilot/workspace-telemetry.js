...
renderCommand=function(){
  ...
  box.innerHTML='<div class="title"><h3>Telemetría del piloto operacional</h3><span>RIGP-OPS-SIM-001</span></div><div class="pad"><div class="metrics">'
    ...
    +'</div><div class="two" style="margin-top:9px"><div class="block"><h4>Velocidad</h4><p>Mediana hasta primera acción: <b>'+(s.median_minutes_to_first_action==null?'sin datos aún':fmtN(s.median_minutes_to_first_action)+' min')+'</b><br>Acciones registradas: '+esc(s.action_count)+'<br>Escalaciones aprobadas: '+esc(s.approved_escalation_count)+' · rechazadas: '+esc(s.rejected_escalation_count)+'</p></div><div class="block"><h4>Actividad por usuario</h4>'+(actors.length?actors.map(a=>'<div class="component"><div><div class="name">'+esc(a.actor_label)+'</div><div class="sub">'+esc(a.pilot_role||'')+' · '+esc(a.touched_task_count)+' tareas</div></div><span class="small">'+esc(a.action_count)+' acciones</span></div>').join(''):'<p>La sesión todavía no registra actividad humana.</p>')+'</div></div></div>';
